...
fn main() {
    ...
    let data_path = if Path::new("../data/test_data.txt").exists() {
        "../data/test_data.txt"
    } else {
        "data/test_data.txt"
    };
    let matrix = BitMatrix::load_from_file(data_path, 4).expect(&format!("Failed to load test data from {}", data_path));
