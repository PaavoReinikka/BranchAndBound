...
# # append the data 4 times to increase the dataset size
# dense_data = dense_data * 2
# # randomly shuffle the data on both dimensions
# random.shuffle(dense_data)
# for row in dense_data:    random.shuffle(row)

print("Transforming dense data to sparse...")
sparse_data, id_to_name, name_to_id = kingfisher.dense_to_sparse(
    dense_data, column_names
)
...
rules = kingfisher.find_rules_from_data(
    ...
    m_threshold=1.0,  # Minimum p-value (1.0 = return all)
)
